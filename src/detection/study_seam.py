...
def report(res: dict) -> str:
    ...
    L += ["**Geometry check (premise holds):** joining chunk k's right edge to chunk k+1's left edge is nearly "
          "seamless (discontinuity ratio 1.56 on Rec6 port 5|6) while the reverse order shows a seam (2.52) — "
          "consecutive chunks do continue left → right, as `stitch.py` assumes.", "",
          "**Decision:** no recall gain (ceiling unchanged, AP slightly lower) → seam inference is **not** in the "
          "product. Most of these frames are Roboflow copies, so many edge-cut pots were cut by an augmentation "
          "crop whose other half is not in the neighbouring chunk. Re-test on un-augmented frames (v2b test) with EXP-002.", "",
