...
class EvidenceGatherer:
    ...
    def gather(self, frame: np.ndarray, det: Detection, nadir: str | None = None) -> Evidence:
        ...
        score = self._fuse(det.conf, relook, proof)
        notes = self._notes(det, relook, proof)
        return Evidence(relook=relook, shadow=proof, echo_ratio=proof.echo_ratio,
                        evidence_score=round(score, 3), notes=notes)

    @staticmethod
    def _fuse(det_conf: float, relook: RelookResult, proof: ShadowProof) -> float:
    ...
    @staticmethod
    def _notes(det: Detection, relook: RelookResult, proof: ShadowProof) -> list[str]:
        notes: list[str] = []
        if relook.found:
            verb = "persisted" if relook.conf >= det.conf else "weakened"
