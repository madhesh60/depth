...
| **OpenCV output drives actions — shown by counterfactual** | the same survey re-run with each OpenCV output withheld: without Stage 1, **193 of 264** pins fall outside their own error circle and 13 of 63 re-survey passes regroup; without the shadow, nothing changes (evidence only) | [`docs/causal_trace.md`](docs/causal_trace.md) |
